<?php
	...
	if(isset($code) && trim($code) != '' ){
		...
		$urli = WEPAY_DOMAIN.WEPAY_ACCESSTOKEN_URI."?client_id=".WEPAY_CLIENT_ID."&redirect_uri=".WEPAY_REDIRECT_URI."&client_secret=".WEPAY_CLIENT_SECRET."&code=".$code;
		//$url = WEPAY_DOMAIN_LIVE.WEPAY_ACCESSTOKEN_URI."?client_id=".WEPAY_CLIENT_ID_LIVE_TUE."&redirect_uri=".WEPAY_REDIRECT_URI."&client_secret=".WEPAY_CLIENT_SECRET_LIVE_TUE."&code=".$code;//XXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
		fwrite($fp3, "URL --> " . $urli ."\n\n");
		...
		*/
		...
		if(isset($jsonReturn->user_id) && isset($jsonReturn->access_token) && isset($jsonReturn->token_type))		{
			...
			*/		
			...
			$arguments = (array('name' => BHLEAGUE_ACCOUNT_DEMO, 'description' => BHLEAGUE_DESC_DEMO));
			//$arguments = (array('name' => BHLEAGUE_ACCOUNT_TUE, 'description' => BHLEAGUE_DESC_TUE));//XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
			...
			*/			
			...
			//$arguments = array('account_id'=>$jsonReturn_payment->account_id, 'amount'=>'150.00', 'short_description'=>BHLEAGUE_DESC_TUE, 'type'=>'DONATION', 'redirect_uri'=>WEPAY_THANKYOU_URI_V3);//XXXXXXXXXXXXXXXXXXXXXXXXXXXX
			curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
			curl_setopt($ch, CURLOPT_POST, true);
			...
			//$ch = curl_init(WEPAY_DOMAIN2_LIVE."user");//XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
			//$arguments = array('account_id'=>$jsonReturn_payment->account_id, 'amount'=>'150.00', 'short_description'=>BHLEAGUE_DESC_TUE, 'type'=>'DONATION', 'redirect_uri'=>WEPAY_THANKYOU_URI);
			curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
			curl_setopt($ch, CURLOPT_POST, true);
			curl_setopt($ch, CURLOPT_HTTPHEADER, array("Authorization: Bearer ".$jsonReturn->access_token));
			curl_setopt($ch, CURLOPT_CAINFO,  'cacert.pem');
			...
			//$sql = "SELECT wepay_user_id from `bhleague`.`wepay_accounts` where wepay_user_id ='".$jsonReturn->user_id."' LIMIT 1";
