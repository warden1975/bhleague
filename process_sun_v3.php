<?php
	...
	if(isset($code) && trim($code) != '' ){
		...
		//$url = WEPAY_DOMAIN.WEPAY_ACCESSTOKEN_URI."?client_id=".WEPAY_CLIENT_ID_SUN."&redirect_uri=".WEPAY_REDIRECT_URI_SUN."&client_secret=".WEPAY_CLIENT_SECRET_SUN."&code=".$code;
		$url = WEPAY_DOMAIN_LIVE.WEPAY_ACCESSTOKEN_URI."?client_id=".WEPAY_CLIENT_ID_LIVE_SUN."&redirect_uri=".WEPAY_REDIRECT_URI_LIVE_SUN."&client_secret=".WEPAY_CLIENT_SECRE_LIVE_SUN."&code=".$code;//XXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
		...
		*/
		...
		if(isset($jsonReturn->user_id) && isset($jsonReturn->access_token) && isset($jsonReturn->token_type))		{
			...
			*/		
			...
			//$arguments = (array('name' => BHLEAGUE_ACCOUNT_DEMO_SUN, 'description' => BHLEAGUE_DESC_DEMO_SUN));
			$arguments = (array('name' => BHLEAGUE_ACCOUNT_SUN, 'description' => BHLEAGUE_DESC_SUN));//XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
			...
			*/			
			...
			//$arguments = array('account_id'=>$jsonReturn_payment->account_id, 'amount'=>'150.00', 'short_description'=>BHLEAGUE_ACCOUNT_DEMO_SUN, 'type'=>'DONATION', 'redirect_uri'=>WEPAY_THANKYOU_URI);
			$arguments = array('account_id'=>$jsonReturn_payment->account_id, 'amount'=>'150.00', 'short_description'=>BHLEAGUE_DESC_DEMO, 'type'=>'DONATION', 'redirect_uri'=>WEPAY_THANKYOU_URI_V3);//XXXXXXXXXXXXXXXXXXXXXXXXXXXX
			curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
			curl_setopt($ch, CURLOPT_POST, true);
